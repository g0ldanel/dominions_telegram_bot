  ...
  def select_game!
    games = Dominions4botController::PORTS.map do |port|
      if Game.exists? port: port
        [{text: "Borrar #{port}", callback_data: "delete_game! #{port}"},{text: "#{port} Who is who?", callback_data: "who_is_who #{port}" },{text: "Botonaso!", callback_data: "botonaso! #{port}" },{text: "Reinicia!", callback_data: "reinicia! #{port}" }]
      else
